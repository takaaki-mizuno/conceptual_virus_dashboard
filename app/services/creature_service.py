...
class CreatureService(object):
    ...
    def get_active_creatures(self, db: Session) -> [Creature]:
        ...
        return db.query(Creature).filter(Creature.last_ping_sent_at > threshold_time, Creature.is_active == True).all()

    def get_creature(self, db: Session, creature_id: int) -> Optional[Tuple[Creature, Snapshot]]:
        current_time = int(time.time())
        threshold_time = current_time - self.THRESHOLD_DURATION
        creature = db.query(Creature).filter(Creature.id == creature_id, Creature.last_ping_sent_at > threshold_time,
                                             Creature.is_active == True).first()
        if creature is None:
            return None

        snapshot = db.query(Snapshot).filter(Snapshot.creature_id == creature_id).order_by(
    ...
    def register_creature(self, db: Session, ip_address: str, identity_key: str, status: dict) -> Optional[
        ...
        if creature is not None:
            if creature.identity_key == identity_key:
                db.query(Creature).filter(Creature.id == creature.id).update({"last_ping_sent_at": current_time})
                db.refresh(creature)
            else:
